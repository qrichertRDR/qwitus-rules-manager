# ---
# Name: Rente selon enfant (nouveau)
# Short Name: ri_salaire_tranche_enfant_2019
# Type: benefit
# ---
...
def ri_enfant():
    ...
    def specificites():
        ...
        # GESTION ENFANT
        # 0:1$2:5$6:+ -> ['0:1', ...]
        classes_enf = param_01z_classes_enfant()
        classes_enf = classes_enf.split(car_sep_classes)
        for i, classe in enumerate(classes_enf):
            a, b = classe.split(car_sep_inf_sup)
            a = Decimal(a)
            if b == '+':
                b = '99'
            b = Decimal(b)
        ...
        # Paramètres
        base_r1 = param_03a_base_r1()
        base_r1_sep = base_r1 is not None and base_r1.lower() in \
                      ['brut', 'net']

        # %TA/%TB/%TC$%TA/%TB/%TC$...
        pcts_z_enf = ['0/0/0'] * len(classes_enf)
        pcts_z_enf_str = car_sep_classes.join(pcts_z_enf)

        pcts_r1_enf = param_03a_pcts_r1() if compl_0a_gestion_r1() else pcts_z_enf_str
        if base_r1_sep:
            pcts_r1_enf = pcts_r1_enf.split(car_sep_classes)

        pcts_r2_enf = param_03b_pcts_r2() if compl_0b_gestion_r2() else pcts_z_enf_str
        pcts_r2_enf = pcts_r2_enf.split(car_sep_classes)

        pcts_r3_enfant = param_03c_pcts_r3() if compl_0c_gestion_r3() else pcts_z_enf_str
        pcts_r3_enfant = pcts_r3_enfant.split(car_sep_classes)

        base_rat = param_03d_base_rat()
        pcts_rat_enf = param_03d_pcts_rat() if compl_0d_gestion_rat() else pcts_z_enf_str
        base_rat_sep = base_rat is not None and base_rat.lower() in \
                       ['brut', 'net']
        if base_rat_sep:
            pcts_rat_enf = pcts_rat_enf.split(car_sep_classes)

        pcts_lim = param_03e_pcts_limite()

        nb_enf_a_charge = compl_nombre_d_enfants_a_charge()
        if nb_enf_a_charge is None:
            message_none = "Saisir un nombre d'enfants."
            ajouter_erreur(message_none)
            return

        if base_r1_sep:
            pcts_r1 = enfant_vers_pcts(
                classes_enf, pcts_r1_enf, nb_enf_a_charge
            )
        else:
            pcts_r1 = pcts_r1_enf

        pcts_r2 = enfant_vers_pcts(
            classes_enf, pcts_r2_enf, nb_enf_a_charge
        )

        pcts_r3 = enfant_vers_pcts(
            classes_enf, pcts_r3_enfant, nb_enf_a_charge
        )

        if base_rat_sep:
            pcts_rat = enfant_vers_pcts(
                classes_enf, pcts_rat_enf, nb_enf_a_charge
            )
        else:
            pcts_rat = pcts_rat_enf
    ...
    return rule_ri_salaire_tranche(
        ...
        rass2_n=rass2_n, rass3_b=rass3_b, rass3_n=rass3_n, raatss_b=rassat_b,
        raatss_n=rassat_n, tip=tip, pcts_r1=pcts_r1, pcts_r2=pcts_r2,
        pcts_r3=pcts_r3, pcts_rat=pcts_rat, pcts_lim=pcts_lim
    )

return ri_enfant() # DECOMMENTER SOUS COOG
