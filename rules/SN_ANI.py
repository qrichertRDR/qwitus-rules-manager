# ---
# Name: Salaire net par tranche de salaire (ANI)
# Short Name: SN_ANI
# Type: benefit
# ---
...
if montant_mi_temps:
    ...
    if salaire_journalier > limite_mi_temps:
        ...
        description += 'Limitation du mi temps thérapeuthique (salaire de reference - IJSS - montant percu en mi-temps: %s€ - %s€ - %s€ = %s€' % (
            salaire_reference_journalier, IJSS, montant_mi_temps, salaire_journalier)
    else:
    ...
    elif compl_allocation_journaliere_de_pole_emploi() < salaire_journalier + IJSS:
        salaire_journalier_base = compl_allocation_journaliere_de_pole_emploi() - IJSS
        description += "Limitation à l'allocation journaliere de pole emploi soit %s€\n" % max(salaire_journalier_base, 0)
    else:
