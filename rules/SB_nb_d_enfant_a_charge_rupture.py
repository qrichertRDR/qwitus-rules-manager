# ---
# Name: Salaire brut par tranche de salaire selon nb d 'enfant à charge avec rupture
# Short Name: SB_nb_d_enfant_a_charge_rupture
# Type: benefit
# ---
...
if nb_enfant_a_charge == 0:
    if not rupture:
        ...
        pourcentages = param_6_ta_tb_tc_0_enfant_rupture()
elif nb_enfant_a_charge == 1:
    if not rupture:
        pourcentages = param_2_pourcentage_ta_tb_tc_1_enfant()
    else:
        pourcentages = param_7_ta_tb_tc_1_enfant_rupture()
elif nb_enfant_a_charge == 2:
    if not rupture:
        pourcentages = param_3_pourcentage_ta_tb_tc_2_enfant()
    else:
        pourcentages = param_8_ta_tb_tc_2_enfant_rupture()
elif nb_enfant_a_charge == 3:
    if not rupture:
        pourcentages = param_4_pourcentage_ta_tb_tc_3_enfant()
    else:
        pourcentages = param_9_ta_tb_tc_3_enfant_rupture()
elif nb_enfant_a_charge >= 4:
    if not rupture:
        pourcentages = param_5_pourcentage_ta_tb_tc_4_enfant_et_plus()
    else:
        pourcentages = param_10_ta_tb_tc_4_enfant_et_plus_rupture()
ajouter_info(rupture)
ajouter_info(pourcentages)
