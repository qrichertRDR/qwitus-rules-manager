# ---
# Name: Indemnité Journalière MENSU CCN Labo
# Short Name: mensu_ccnlabo
# Type: benefit
# ---
...
def specificites_palier_standard():
    ...
    # Paramètres
    complement = False
    reference_palier = 'dei'
    
    base_av_rup_0 = 'brut'
    base_av_rup_1 = 'brut'
    base_ap_rup_0 = 'brut'
    base_ap_rup_1 = 'brut'
    pcts_base_av_rup_0='90/90/90'
    pcts_base_av_rup_1='66.66/66.66/66.66'
    pcts_base_ap_rup_0='90/90/90'
    pcts_base_ap_rup_1='66.66/66.66/66.66'
    duree_0=30
    duree_max='60_JOURS_DEI'
    jour_max=60
    if anciennete<1:
        pcts_base_av_rup_0='0/0/0'
        pcts_base_av_rup_1='0/0/0'
        pcts_base_ap_rup_0='0/0/0'
        pcts_base_ap_rup_1='0/0/0'
        ...
        duree_0=40
        duree_max='80_JOURS_DEI'
        jour_max=80
        
    elif anciennete>=11 and anciennete<16:
        duree_0=50
        duree_max='100_JOURS_DEI'
        jour_max=100
                
    elif anciennete>=16 and anciennete<21:
        duree_0=60
        duree_max='120_JOURS_DEI'
        jour_max=120
                    
    elif anciennete>=21 and anciennete<26:
        duree_0=70
        duree_max='140_JOURS_DEI'
        jour_max=140
                    
    elif anciennete>=26 and anciennete<31:
        duree_0=80
        duree_max='160_JOURS_DEI'
        jour_max=160
                    
    elif anciennete>=31 and anciennete<46:
        duree_0=90
        duree_max='180_JOURS_DEI'
        jour_max=180
                    
    message_debug(duree_max)

    fin_franchise = date_fin_franchise()
    dei = ajouter_jours(fin_franchise, 1)
    date_lim_max =ajouter_jours(dei, jour_max-1)
    message_debug(date_fin_franchise())
    date_lim_1=ajouter_jours(date_fin_franchise(), duree_0)
    message_debug(date_lim_1)
    duree_0=duree_0-1
 
    return [complement, reference_palier, duree_0, [base_av_rup_0, base_av_rup_1], [pcts_base_av_rup_0, pcts_base_av_rup_1], [base_ap_rup_0, base_ap_rup_1], [pcts_base_ap_rup_0, pcts_base_ap_rup_1], duree_max, anciennete, date_lim_max, date_lim_1]

def param_a01c_prime():
    return 'oui'
def param_a01a_duree_presta():
    return specificites_palier_standard()[7]
def param_a02c_lim_av_rup():
    return 'non'
def param_a02d_pcts_lim_av_rup():
    return ''    
def param_a02e_rv_lim_av_rup():
    return False 
def param_a03c_lim_ap_rup():
    return 'net' 
def param_a03d_pcts_lim_ap_rup():
    return '100/100/0'
def param_a03e_rv_lim_ap_rup():
    return False
def param_a04a_red_ib_mt():
    return 'indemnite_base' 
def param_a04b_lim_mt():
    return 'reference' 
def param_a04c_rv_lim_mt():
    return False
def param_a05a_assiette_rv():
    return 'prestation'
def param_a05b_freq_rv():
    return '01/01/annuel'
def param_a05c_indice_rv():
    return 'agirc_arrco' 
def param_a05d_premiere_rv():
    return '01/01'        
def param_a05e_date_ref_rv():
    return 'dat'        
def param_a05f_nbj_rv():
    return 0        
def param_a06a_type_deduction_recccn():
    return 'sans_objet'
def param_a07_convertir_ijss_brut_vers_net():
    return False
...
def ij_un_palier(specificites):
    ...
    message_debug('**'+param_a01a_duree_presta())
    message_debug('**'+duree)
...
def ij_un_palier_standard():
    ...
    if (dateanciennete is None):
        ajouter_erreur("La date d'ancienneté n'a pas été saisie.")
        return True
    ...
    # ---------------------------------------------
    for i_res in resultats:
        i_res['description']= 'DATE FIN MENSUALISATION Palier 2: ' + formater_date(specificites_palier_standard()[9]) + ' \n\n' + i_res['description']
        i_res['description']= 'DATE FIN MENSUALISATION Palier 1: ' + formater_date(specificites_palier_standard()[10]) + ' \n' + i_res['description']
        i_res['description']= 'ANCIENNETE: ' + str(round(specificites_palier_standard()[8], 2)) + ' année(s) \n' + i_res['description']
