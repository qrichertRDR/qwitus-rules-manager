# ---
# Name: Franchise relais complément CCN Labo
# Short Name: fr_reccn_labo
# Type: benefit_deductible
# ---

def param_1_type_franchise():
    return 'continue'
def param_2_franchise_accident():
    return 3
def param_2a_condition_accident():
    return 'aucune'
def param_2b_rachat_accident():
    return None
def param_3_franchise_accident_travail():
    return 0
def param_3a_condition_accident_travail():
    return 'aucune'
def param_3b_rachat_accident_travail():
    return None
def param_4_franchise_maladie():
    return 3
def param_4a_condition_maladie():
    return 'aucune'
def param_4b_rachat_maladie():
    return None        
def param_5_franchise_maladie_professionnelle():
    return 0
def param_5a_condition_maladie_professionnelle():
    return 'aucune'
def param_5b_rachat_maladie_professionnelle():
    return None     
def param_6_franchise_maternite():
    return 0
def param_6a_condition_maternite():
    return 'aucune'
def param_6b_rachat_maternite():
    return None     
...
# RETIRER PARAMETRES SOUS COOG
def fr_jours_arret(gestion_declaration_tardive=True):
    '''
    :param param_1_type_franchise: type de franchise
    :type param_1_type_franchise: () -> str

    :param param_2_franchise_accident: franchise en cas d'accident
    :type param_2_franchise_accident: () -> int

    :param param_3_franchise_accident_travail: franchise en cas d'accident du travail
    :type param_3_franchise_accident_travail: () -> int

    :param param_4_franchise_maladie: franchise en cas de maladie
    :type param_4_franchise_maladie: () -> int

    :param param_5_franchise_maladie_professionnelle: franchise en cas de maladie professionnelle
    :type param_5_franchise_maladie_professionnelle: () -> int

    :param param_6_franchise_maternite: franchise en cas de maternité
    :type param_6_franchise_maternite: () -> int

    :param param_7_regle_rachat: règle de rachat de franchise
    :type param_7_regle_rachat: () -> str

    :param param_8_franchise_rachat: franchise en cas de rachat
    :type param_8_franchise_rachat: () -> int
    ...
    type_franchise = param_1_type_franchise()

    franchise_accident = param_2_franchise_accident()
    condition_accident = param_2a_condition_accident()
    rachat_accident = param_2b_rachat_accident()

    franchise_accident_travail = param_3_franchise_accident_travail()
    condition_accident_travail = param_3a_condition_accident_travail()
    rachat_accident_travail = param_3b_rachat_accident_travail()

    franchise_maladie = param_4_franchise_maladie()
    condition_maladie = param_4a_condition_maladie()
    rachat_maladie = param_4b_rachat_maladie()

    franchise_maladie_pro = param_5_franchise_maladie_professionnelle()
    condition_maladie_pro = param_5a_condition_maladie_professionnelle()
    rachat_maladie_pro = param_5b_rachat_maladie_professionnelle()

    franchise_maternite = param_6_franchise_maternite()
    condition_maternite = param_6a_condition_maternite()
    rachat_maternite = param_6b_rachat_maternite()
    ...
    if gestion_declaration_tardive and code_decision_eligibilite_prestation() == 'declaration_tardive_avec_penalite':
        ...
        if decla_tardive_variable == 'sans_penalite':
            ...
            fin_franchise_init = fr_jours_arret(False) # éviter infinite recursion depth
        ...
        # DEPLACER DANS ELIGIBILITE ? IMPOSSIBLE car eligibilité plus appellée
            ...
            fin_franchise_init = fr_jours_arret(False)  # infinite recursion depth
    ...
    if anciennete<1:
        fin_franchise = max(ajouter_jours(fin_franchise, 4), ajouter_jours(debut_prejudice, 6))
	
    elif anciennete>=1 and anciennete<6:
        fin_franchise = ajouter_jours(fin_franchise, 30)
		
    elif anciennete>=6 and anciennete<11:
        fin_franchise = ajouter_jours(fin_franchise, 40)
	
    elif anciennete>=11 and anciennete<16:
        fin_franchise = ajouter_jours(fin_franchise, 50)
		
    elif anciennete>=16 and anciennete<21:
        fin_franchise = ajouter_jours(fin_franchise, 60)
		
    elif anciennete>=21 and anciennete<26:
        fin_franchise = ajouter_jours(fin_franchise, 70)
		
    elif anciennete>=26 and anciennete<31:
        fin_franchise = ajouter_jours(fin_franchise, 80)
		
    elif anciennete>=31 and anciennete<46:
        fin_franchise = ajouter_jours(fin_franchise, 90)
	# Fin modif CCN LABO
